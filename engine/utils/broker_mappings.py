    ...
    try:
        ...
        if is_monthly:
            ...
            m_char = (
                f"{m_int}"
                if m_int < 10
                else "O" if m_int == 10 else "N" if m_int == 11 else "D"
            )
...
def generate_basket_html(legs, broker="Zerodha"):
    ...
    for leg in legs:
        ...
        z_sym = (
            convert_yahoo_to_zerodha(y_sym)
            if leg["type"] != "STOCK"
            else clean_symbol_for_broker(leg.get("contractSymbol"))
        )
