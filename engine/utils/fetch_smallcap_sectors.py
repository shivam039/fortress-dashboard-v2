...
def fetch_sectors():
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            print(
                f"[{i+1}/{len(tickers)}] {ticker}: {mapped_sector} ({sector} - {industry})"
            )
...
def map_sector(sector, industry, ticker):
    ...
    if (
        "Financial" in sector
        or "Credit" in industry
        or "Asset Management" in industry
    ...
    if "Basic Materials" in sector:
        if (
            "Steel" in industry
            or "Metal" in industry
            or "Mining" in industry
            or "Aluminum" in industry
            or "Copper" in industry
            or "Gold" in industry
        ):
            return "Metals"
    ...
    if "Consumer Defensive" in sector:
        if (
            "Beverages" in industry
            or "Food" in industry
            or "Household" in industry
            or "Personal" in industry
            or "Tobacco" in industry
        ):
            return "FMCG"
    ...
    if "Industrials" in sector:
        ...
        if (
            ...
            or "Transport" in industry
            or "Shipping" in industry
            or "Railroads" in industry
        ):
    ...
    if "Communication Services" in sector:
        ...
        if (
            "Media" in industry
            or "Entertainment" in industry
            or "Advertising" in industry
        ):
            return "Media"
