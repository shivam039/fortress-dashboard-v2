...
def score_mf_fund(row: dict, category: str = "Equity") -> dict:
    ...
    alpha_score = _clamp(
        25 + min(alpha * 2.0, 15) if alpha >= 0 else 25 + max(alpha * 3.0, -25), 0, 25
    )
...
def score_commodity(
    ...
    # ── 2. Momentum (0-25) ───────────────────────────────────────────────
    ret_1m = (
        float(((close.iloc[-1] / close.iloc[-min(21, n)]) - 1) * 100)
        if n >= 21
        else 0.0
    )
    ret_3m = (
        float(((close.iloc[-1] / close.iloc[-min(63, n)]) - 1) * 100)
        if n >= 63
        else 0.0
    )
    ret_6m = (
        float(((close.iloc[-1] / close.iloc[-min(126, n)]) - 1) * 100)
        if n >= 126
        else 0.0
    )
    ...
    if not global_df.empty:
        ...
        g_1m = (
            float(((g_close.iloc[-1] / g_close.iloc[-min(21, len(g_close))]) - 1) * 100)
            if len(g_close) >= 21
            else 0
        )
    ...
    # ── Final score ───────────────────────────────────────────────────────
    total = _clamp(
        trend_score + momentum_score + vol_score + arb_score + global_bonus, 0, 100
    )
    label, emoji = _label(total)
    ...
    if spread_pct < -2:
        parts.append(
            f"local underpriced vs global by {abs(spread_pct):.1f}% (buy edge)"
        )
    elif spread_pct > 3:
        parts.append(
            f"local overpriced vs global by {spread_pct:.1f}% (reduce exposure)"
        )
    ...
    return {
        ...
        "ATR Regime": (
            "Coiling 🔥"
            if vol_score >= 18
            else ("Normal" if vol_score >= 8 else "Expanded ⚠️")
        ),
        "Sub-Scores": {
