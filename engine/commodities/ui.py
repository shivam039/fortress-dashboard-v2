...
def render(broker_choice="Zerodha"):
    ...
    commodity = st.sidebar.selectbox(
        "Commodity", ["All", "Gold", "Silver", "Crude", "Copper"]
    )
    ...
    with st.expander("🔥 Conviction & Spread Heatmap", expanded=False):
        heat_data = df.set_index("Commodity")[
            ["Conviction Score", "Spread %", "1M Return %", "3M Return %"]
        ]
        heat = px.imshow(
            heat_data.T,
            aspect="auto",
        ...
        heat.update_xaxes(
            tickvals=list(range(len(df))), ticktext=df["Commodity"].tolist()
        )
    ...
    with st.expander("📈 Return Comparison", expanded=False):
        ret_cols = [
            c for c in ["1M Return %", "3M Return %", "6M Return %"] if c in df.columns
        ]
        if ret_cols:
            fig = go.Figure()
            for col in ret_cols:
