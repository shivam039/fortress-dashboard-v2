...
def fetch_price_series(
    ...
    Set ``force_refresh`` when a broadcaster or scheduled job needs a live
    scan rather than the last cached commodity snapshot.
    """
...
def compute_atr(df: pd.DataFrame, window: int = 14) -> float:
    ...
    return (
        float(tr.rolling(window).mean().iloc[-1])
        if len(tr) > window
        else float(tr.mean())
    )


def build_commodities_frame(
    selection: Optional[str] = None, force_refresh: bool = False
) -> pd.DataFrame:
    from utils.conviction_engine import score_commodity

    usdinr = fetch_price_series("INR=X", period="1mo", force_refresh=force_refresh)
    ...
    for name, cfg in COMMODITY_MAP.items():
        if selection and name != selection:
            continue
        global_df = fetch_price_series(
            cfg["global"], period="6mo", force_refresh=force_refresh
        )
        local_df = fetch_price_series(
            cfg["local"], period="6mo", force_refresh=force_refresh
        )
