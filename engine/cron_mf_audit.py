...
from mf_lab.services.alerts import (
    check_integrity_rules,
    generate_smart_alerts,
    send_telegram_alert,
)

# Services
from mf_lab.services.config import LOCK_FILE, logger
from mf_lab.services.data import discover_funds, fetch_benchmark_data, fetch_fund_nav
...
def run_audit(limit=None):
    ...
    try:
        ...
        scan_id = register_scan(
            timestamp, universe="Mutual Funds", status="In Progress"
        )
        ...
        for i, c in enumerate(candidates):
            try:
                ...
                if len(fund_df) < 750:
                    continue  # Min history
                if fund_df["nav"].iloc[-1] < 10:
                    continue  # Low quality/penny fund filter
                ...
                if metrics:
                    ...
                    # Drift Check
                    integrity, drift_status, drift_msg = check_integrity_rules(
                        metrics, cat
                    )
        ...
        if results_data:
            ...
            log_audit(
                "MF Top10",
                "Mutual Funds",
                f"Top consistent schemes: {', '.join(top10['symbol'].astype(str).tolist())}",
            )
