...
import pandas as pd
import requests
from mf_lab.services.config import API_TIMEOUT, RETRY_DELAY, logger
...
def _parse_scheme_list(
    raw_schemes: List[Dict], direct_growth_only: bool = True
) -> List[Dict[str, str]]:
    """
...
def fetch_all_schemes_from_api(max_retries: int = 3) -> List[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"Fetching scheme catalog from mfapi.in (attempt {attempt + 1}/{max_retries})..."
            )
            ...
            logger.info(
                f"✓ Filtered to {len(enriched)} Direct-Growth schemes and categorized"
            )
            ...
            logger.warning(
                f"API fetch failed (attempt {attempt + 1}/{max_retries}): {e}"
            )
...
def get_all_schemes_cached(force_refresh: bool = False) -> pd.DataFrame:
    ...
    try:
        ...
        from utils.db import _exec, _read_df
        ...
        if not force_refresh:
            try:
                ...
                if not df.empty:
                    logger.info(
                        f"✓ Loaded {len(df)} schemes from monthly cache (cached {df['cached_date'].iloc[0]})"
                    )
                    return df[
                        [
                            "scheme_code",
        ...
        logger.info(
            "Fetching fresh scheme list from API (cache expired or forced refresh)..."
        )
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                logger.debug(
                    f"Batch compute error for {row['type']} / {row['category']}: {batch_err}"
                )

        logger.info(
            f"✓ Pre-computed {len(df)} category/type batches for instant filtering"
        )
...
def get_batch_stats() -> Dict[str, any]:
    ...
    **O(1)** - pulls from mf_scheme_batches table (no filtering needed).
    ...
    try:
        ...
        result["_metadata"] = {"total_schemes": total_schemes, "total_batches": len(df)}
...
def get_batch_filtered_schemes(
    scheme_type: str = None, category: str = None
) -> pd.DataFrame:
    """
...
def parallel_fetch_scheme_navs(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(fetch_fund_nav, code): code for code in scheme_codes}
...
def get_distinct_categories_for_type(scheme_type: str) -> List[str]:
    ...
    try:
        ...
        logger.debug(
            f"Batch table unavailable for type '{scheme_type}', falling back to cached schemes: {e}"
        )
