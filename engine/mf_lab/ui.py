...
def render():
    ...
    tab_analysis, tab_discovery = st.tabs(
        ["📊 Consistency Analysis", "🔍 Scheme Browser (4000+ Schemes)"]
    )
    ...
    with tab_discovery:
        try:
            ...
            st.warning(
                "Scheme Browser is temporarily unavailable, but cached MF analysis can still be used."
            )
...
def _render_consistency_analysis():
    ...
    BACKEND_URL = st.session_state.get(
        "fastapi_url", os.environ.get("FORTRESS_API_URL", "http://127.0.0.1:8000")
    )
    JOB_OPTIONS = {
    ...
    categories = st.sidebar.multiselect(
        "Category", ["Equity", "Debt", "Hybrid"], default=["Equity", "Debt", "Hybrid"]
    )
    min_sharpe = st.sidebar.number_input(
        "Min Sharpe", min_value=0.0, value=0.5, step=0.1
    )
    ...
    with col_db1:
        ...
        st.caption(
            "This view can load fully from cached database results without triggering any backend recalculation."
        )
    with col_db2:
        db_only_refresh = st.button(
            "Load From DB", use_container_width=True, key="mf_db_refresh"
        )
        if db_only_refresh:
        ...
        with st.expander("⚙️ Server-Side Data Jobs", expanded=False):
            ...
            with col_force:
                force_refresh = st.checkbox(
                    "Force Refresh", value=False, key="mf_force_refresh"
                )

            if st.button(
                "🚀 Trigger Job on Server", type="primary", key="mf_trigger_btn"
            ):
                ...
                try:
                    ...
                    if resp.status_code == 202:
                        ...
                        st.error(
                            f"❌ Server rejected job (HTTP {resp.status_code}): {resp.text}"
                        )
    ...
    if cached_df.empty:
        st.info(
            "No data yet. Click **🚀 Run Full MF Scan** above to automatically scrape the newest AMFI data for this month."
        )
        return
    ...
    if filtered.empty:
        st.warning(
            "No funds match the current filters. Adjust Sharpe or Category in the sidebar."
        )
    else:
        ...
        if top_db_picks.empty:
            ...
            for cat in ["Equity", "Debt", "Hybrid"]:
                ...
                for sub in sub_cats:
                    st.markdown(f"#### ⤷ {sub}")
                    sub_df = cat_df[cat_df["Sub Category"] == sub].sort_values(
                        "Conviction Score", ascending=False
                    )
        ...
        if not filtered.empty and "Scheme Code" in filtered.columns:
            scheme_choice = st.selectbox(
                "📊 Backtest a fund vs Nifty 50", filtered["Scheme Code"].tolist()
            )
            if scheme_choice:
