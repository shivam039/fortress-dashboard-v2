...
def render_scheme_discovery_tab():
    ...
    with tab_by_type:
        ...
        if not scheme_types:
            ...
            with col1:
                st.write(
                    f"**{len(type_schemes)} {selected_type} Schemes** (from pre-computed batch)"
                )

            with col2:
                display_format = st.radio(
                    "Display:", ["Table", "List"], horizontal=True, key="type_format"
                )
        ...
        if not available_types:
            ...
            with col1:
                selected_type = st.selectbox(
                    "Select Fund Type:", available_types, key="cat_type_selector"
                )
            ...
            if selected_category:
                ...
                cat_schemes = get_batch_filtered_schemes(
                    scheme_type=selected_type, category=selected_category
                )

                if cat_schemes.empty:
                    st.info(
                        f"No schemes found for {selected_type} → {selected_category}"
                    )
                else:
                    ...
                    st.dataframe(
                        ...
                        hide_index=True,
                        column_config={
                            "scheme_name": st.column_config.TextColumn(
                                "Scheme Name", width="large"
                            ),
                            "amc_name": st.column_config.TextColumn(
                                "AMC", width="medium"
                            ),
                            "scheme_code": st.column_config.NumberColumn(
                                "Code", width="small"
                            ),
    ...
    with tab_search:
        ...
        search_query = st.text_input(
            ...
            placeholder="e.g., 'Parag Parikh', 'Nifty 50', '100100'",
        )
        ...
        if search_query:
            ...
            if results.empty:
                ...
                if selected_schemes and st.button("Run Analysis on Selected Schemes"):
                    selected_data = results.loc[
                        selected_schemes, ["scheme_code", "scheme_name"]
                    ]
                    st.session_state["selected_schemes_for_analysis"] = (
                        selected_data.to_dict("records")
                    )
                    st.success(
                        f"✅ Selected {len(selected_schemes)} schemes for analysis"
                    )
...
def render_selected_schemes_analysis():
    ...
    with st.spinner("Fetching NAV history and computing performance metrics..."):
        try:
            from mf_lab.logic import fetch_mf_snapshot

            analysis_df = fetch_mf_snapshot(scheme_codes)
    ...
    if analysis_df.empty:
        st.warning(
            "⚠️ Could not fetch data or compute metrics for the selected schemes. Please verify the scheme codes."
        )
    else:
        ...
        st.dataframe(
            analysis_df,
            column_config={
                "Scheme Code": st.column_config.TextColumn("Code"),
                "Scheme": st.column_config.TextColumn("Scheme Name"),
                "NAV": st.column_config.NumberColumn("NAV", format="₹%.2f"),
                "1Y Return": st.column_config.NumberColumn(
                    "1Y Return", format="%.2f%%"
                ),
                "3Y Return": st.column_config.NumberColumn(
                    "3Y Return", format="%.2f%%"
                ),
                "5Y Return": st.column_config.NumberColumn(
                    "5Y Return", format="%.2f%%"
                ),
                "Volatility": st.column_config.NumberColumn(
                    "Volatility", format="%.2f%%"
                ),
                "Downside Deviation": st.column_config.NumberColumn(
                    "Downside Deviation", format="%.2f%%"
                ),
                "Sharpe": st.column_config.NumberColumn("Sharpe Ratio", format="%.2f"),
                "Sortino": st.column_config.NumberColumn(
                    "Sortino Ratio", format="%.2f"
                ),
                "Consistency Score": st.column_config.ProgressColumn(
                    "Consistency Score",
                    help="Fortress proprietary Mutual Fund consistency score",
                    format="%.1f",
                    min_value=0.0,
                    max_value=100.0,
                ),
