...
def fetch_market_pulse_data():
    ...
    try:
        ...
        if is_multi:
            ...
            nifty_df = (
                all_data.copy()
            )  # Assume flat is Nifty if that's what we got? Unlikely with VIX in list.
...
def get_current_regime() -> dict:
    ...
    try:
        ...
        nifty_close = (
            data["^NSEI"]["Close"].dropna()
            if "^NSEI" in data.columns.levels[0]
            else pd.Series(dtype=float)
        )
        vix_close = (
            data["^INDIAVIX"]["Close"].dropna()
            if "^INDIAVIX" in data.columns.levels[0]
            else pd.Series(dtype=float)
        )
        ...
        nifty_ema200 = (
            float(ta.ema(nifty_close, 200).iloc[-1]) if len(nifty_close) >= 200 else 0.0
        )
        nifty_ema50 = (
            float(ta.ema(nifty_close, 50).iloc[-1]) if len(nifty_close) >= 50 else 0.0
        )
