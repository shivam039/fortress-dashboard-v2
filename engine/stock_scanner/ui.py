...
def get_column_config(display_cols, broker_choice):
    ...
    for col in display_cols:
        ...
        if col == "Actions":
            ...
            st_column_config[col] = st.column_config.NumberColumn(
                cfg["label"], format=fmt
            )
        else:
...
def render_sidebar():
    ...
    if "portfolio_val" not in st.session_state:
        st.session_state["portfolio_val"] = 1000000
    if "risk_pct_slider" not in st.session_state:
        st.session_state["risk_pct_slider"] = 1.0
    if "selected_universe" not in st.session_state:
        st.session_state["selected_universe"] = list(TICKER_GROUPS.keys())[0]

    portfolio_val = st.sidebar.number_input(
        "Portfolio Value (₹)", value=1000000, step=50000, key="portfolio_val"
    )
    risk_pct = (
        st.sidebar.slider(
            "Risk Per Trade (%)", 0.5, 3.0, 1.0, 0.1, key="risk_pct_slider"
        )
        / 100
    )
    ...
    broker_choice = st.sidebar.selectbox(
        "Preferred Broker", ["Zerodha", "Dhan"], key="broker_choice"
    )

    selected_universe = st.sidebar.selectbox(
        "Select Index", list(TICKER_GROUPS.keys()), key="selected_universe"
    )
    ...
    liquidity_cr_min = st.sidebar.number_input(
        "Liquidity gate (₹ Cr, 20D avg)", min_value=0.0, value=8.0, step=0.5
    )
    market_cap_cr_min = st.sidebar.number_input(
        "Market cap gate (₹ Cr)", min_value=0.0, value=1500.0, step=50.0
    )
    price_min = st.sidebar.number_input(
        "Minimum price gate (₹)", min_value=0.0, value=80.0, step=5.0
    )
...
def _save_scan(df, universe):
    ...
    scan_id = register_scan(
        timestamp, universe=universe, scan_type="STOCK", status="Completed"
    )
    ...
    log_audit(
        "Scan Completed",
        universe,
        f"Saved {len(df)} records to unified history (ID: {scan_id})",
    )
    return timestamp
...
def _display_scan_results(df, universe, broker_choice, scoring_config, timestamp=None):
    ...
    filtered_out_df = df[~df.get("Quality_Gate_Pass", True)].copy()
    actionable_df = df[df.get("Quality_Gate_Pass", True)].copy()

    st.success(
        f"Scan Complete: {len(actionable_df[actionable_df['Score']>=60])} actionable setups."
    )

    # --- GENERATE ACTIONS COLUMN ---
    # Universal Action Links: Enabled for all scan results (No Verdict Gate)
    df["Actions"] = df.apply(
        lambda row: generate_action_link(row, broker_choice), axis=1
    )
    ...
    if "Sector" in df.columns:
        if "Velocity" not in df.columns:
            df["Velocity"] = pd.to_numeric(df.get("Ret_7D", 0), errors="coerce").fillna(
                0
            ) - pd.to_numeric(df.get("Ret_30D", 0), errors="coerce").fillna(0)
        sector_stats = (
            df.groupby("Sector")
            .agg(
                {
    ...
    display_cols = [
        c
        for c in st.session_state.get("selected_columns", default_full_list)
        if c in df.columns
    ]
    ...
    if universe == "Nifty Smallcap 250":
        ...
        if display_df.empty:
            st.warning(
                "No tickers met the strict criteria (Score >= 60) for Smallcap 250."
            )

    st_column_config = get_column_config(display_cols, broker_choice)

    st.dataframe(
        display_df, width="stretch", height=600, column_config=st_column_config
    )
    ...
    if not filtered_out_df.empty:
        with st.expander(
            ...
            st.dataframe(
                filtered_out_df[filtered_cols], width="stretch", hide_index=True
            )
    ...
    if not df.empty and "Score" in df.columns:
        ...
        df["Conviction_Band"] = df["Score"].apply(
            lambda x: (
                "🔥 High (85+)"
                if x >= 85
                else "🚀 Pass (60-85)" if x >= 60 else "🟡 Watch (<60)"
            )
        )
        heatmap_data = df.pivot_table(
            index="Symbol", columns="Conviction_Band", values="Score", fill_value=0
        )
        sns.heatmap(
            heatmap_data,
            annot=True,
                ...
                try:
                    ...
                    try:
                        ...
                        with local_logic._META_LOCK:
                            for t in chunk:
                                if t in chunk_meta:
                                    local_logic._INFO_CACHE[t] = chunk_meta[t][
                                        "info_json"
                                    ]
                                    news_data = chunk_meta[t]["news_json"]
                                    local_logic._NEWS_CACHE[t] = (
                                        news_data if isinstance(news_data, list) else []
                                    )
                                    local_logic._CAL_CACHE[t] = (
                                        local_logic._safe_dict_to_df(
                                            chunk_meta[t]["cal_json"]
                                        )
                                    )
                                    local_logic._EARN_CACHE[t] = (
                                        local_logic._safe_dict_to_df(
                                            chunk_meta[t]["earn_json"]
                                        )
                                    )
...
def render(
    ...
    search_symbol = st.text_input(
        "🔍 Search Stock (Symbol)", placeholder="e.g., RELIANCE.NS"
    )
        ...
        try:
            ...
            if not search_hist.empty:
                ...
                if search_res:
                    ...
                    search_df["Actions"] = search_df.apply(
                        lambda row: generate_action_link(row, broker_choice), axis=1
                    )
                    ...
                    st.warning(
                        f"Insufficient data or analysis failed for {search_symbol} (Need >210 candles)."
                    )
            else:
    ...
    if execute_scan:
        ...
        log_audit(
            "Scan Started",
            selected_universe,
            f"Scanning {len(tickers)} tickers synchronously",
        )
        st.rerun()
    ...
    st.caption(
        "🛡️ Fortress 95 Pro v9.4 — Dynamic Columns | ATR SL | Analyst Dispersion | Full Logic"
    )
