...
def _apply_symbol_filter(df: pd.DataFrame, search_term: str) -> pd.DataFrame:
    ...
    if "symbol" not in df.columns:
        return df.iloc[0:0]
    return df[
        df["symbol"].astype(str).str.contains(search_term, case=False, na=False)
    ].copy()


def _display_pick_table(
    title: str, df: pd.DataFrame, selected_label: str, search_term: str
):
    with st.expander(title, expanded=True):
        ...
        if not display_cols:
            display_cols = [
                c for c in df.columns if c not in {"id", "scan_id", "raw_data"}
            ]
...
def render():
    ...
    selected_timestamp = st.selectbox(
        ...
        options=options,
        format_func=lambda ts: (
            "Select a scan timestamp" if ts is None else _format_ts_for_display(ts)
        ),
    )
    symbol_search = st.text_input(
        "Search symbol across all tables", placeholder="e.g. RELIANCE"
    )
    ...
    if "pick_type" in df.columns:
        long_term = df[
            _pick_type_mask(df, ["long-term", "long term", "longterm"])
        ].copy()
    ...
    if st.button("Run Backtest for This Scan", key="hist_backtest_btn", type="primary"):
        with st.spinner("Running backtest against Nifty..."):
            ...
            if not bt_results.empty:
                ...
                st.warning(
                    "No backtest data available (needs >7 days history) or no valid picks found in this scan."
                )
