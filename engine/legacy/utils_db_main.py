...
def init_db():
    ...
    try:
        with get_connection() as conn:
            ...
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_scan_history_scan_id ON scan_history_details(scan_id)"
            )
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_entries_scan_sym ON scan_entries(scan_id, symbol)"
            )
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_metrics_scan_sym ON fund_metrics(scan_id, symbol)"
            )
...
def fetch_timestamps(table_name="scan_mf", scan_type=None):
    ...
    try:
        with get_connection() as conn:
            old_scans = pd.read_sql(
                "SELECT DISTINCT timestamp FROM scan_mf ORDER BY timestamp DESC", conn
            )
            if not old_scans.empty:
                # Avoid duplicates
                existing = set(timestamps)
                legacy = [
                    t for t in old_scans["timestamp"].tolist() if t not in existing
                ]
...
def fetch_history_data(table_name, timestamp, scan_type=None):
    ...
    with get_connection() as conn:
        ...
        scan_info = pd.read_sql(
            "SELECT scan_id, scan_type FROM scans WHERE timestamp = ?",
            conn,
            params=(timestamp,),
        )
        ...
        if not scan_info.empty:
            ...
            if db_scan_type in ["STOCK", "OPTIONS", "COMMODITY"]:
                try:
                    ...
                    df = pd.read_sql(
                        "SELECT * FROM scan_history_details WHERE scan_id = ?",
                        conn,
                        params=(scan_id,),
                    )
                    return df
            ...
            if table_name == "scan_commodities" and (
                db_scan_type is None or db_scan_type == "COMMODITY"
            ):
                # Backward compatibility or specific table usage
                try:
                    df = pd.read_sql(
                        "SELECT * FROM scan_commodities WHERE scan_id = ?",
                        conn,
                        params=(scan_id,),
                    )
                    return df
        ...
        try:
            df = pd.read_sql(
                "SELECT * FROM scan_mf WHERE timestamp=?", conn, params=(timestamp,)
            )
            return df
        except:
            return pd.DataFrame()
...
def fetch_symbol_history(table_name, symbol):
    ...
    with get_connection() as conn:
        ...
        try:
            ...
            df_old = pd.read_sql(
                "SELECT timestamp, Score, Price, `Alpha (True)`, Beta, `Tracking Error` FROM scan_mf WHERE Symbol = ?",
                conn,
                params=(symbol,),
            )
        except Exception as e:
...
def log_algo_trade(strategy, symbol, action, details, status="Active"):
    try:
        with get_connection() as conn:
            ...
            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            c.execute(
                "INSERT INTO algo_trade_log (timestamp, strategy_name, symbol, action, details, status) VALUES (?, ?, ?, ?, ?, ?)",
...
def fetch_active_trades():
    try:
        with get_connection() as conn:
            return pd.read_sql(
                "SELECT * FROM algo_trade_log WHERE status='Active'", conn
            )
    except:
        return pd.DataFrame()
