...
def _ensure_scan_history_details_neon():
    ...
    """
    exists_df = _read_df(
        table_exists_query, {"table_name": "scan_history_details"}, ttl="1s"
    )
    if exists_df.empty:
        try:
            _exec("""
            ...
            logger.exception(
                "Failed to create scan_history_details table in Neon: %s", exc
            )
            raise
    ...
    for column_name, column_type in required_columns.items():
        if not _postgres_has_column("scan_history_details", column_name):
            try:
                ...
                logger.info(
                    "Ensured column %s exists on scan_history_details.", column_name
                )
            except Exception as exc:
                logger.warning(
...
def init_db():
    ...
    with _sqlite_connection() as conn:
        ...
        c.execute(
            """CREATE TABLE IF NOT EXISTS audit_logs (timestamp TEXT, action TEXT, universe TEXT, details TEXT)"""
        )
