...
def _preload_global_config():
    ...
    from fortress_config import INDEX_BENCHMARKS, SECTOR_MAP, TICKER_GROUPS
...
if selected_view == "🚀 Live Scanner":
    (
        ...
        broker_choice,
        scoring_config,
    ) = stock_scanner.ui.render_sidebar()
    stock_scanner.ui.render(
        portfolio_val,
        risk_pct,
        selected_universe,
        selected_columns,
    ...
    broker_choice = st.sidebar.selectbox(
        "Preferred Broker", ["Zerodha", "Dhan"], key="comm_broker"
    )
    try:
    ...
    broker_choice = st.sidebar.selectbox(
        "Preferred Broker", ["Zerodha", "Dhan"], key="algo_broker"
    )
    try:
