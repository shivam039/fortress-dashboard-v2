...
def _apply_quality_gates(df, cfg):
    ...
    if market_cap_col:
        gate_conditions[f"MCap<{cfg['market_cap_cr_min']}Cr"] = (
            pd.to_numeric(df.get(market_cap_col), errors="coerce")
            <= cfg["market_cap_cr_min"]
        )
    if debt_col:
        gate_conditions[f"Debt/Equity>{cfg['max_debt_to_equity']}"] = (
            pd.to_numeric(df.get(debt_col), errors="coerce")
    ...
    gate_frame = pd.DataFrame(
        {k: v.fillna(False) for k, v in gate_conditions.items()}, index=df.index
    )
...
def apply_advanced_scoring(df, scoring_config=None):
    ...
    if "Sector" in df.columns and "Ret_90D" in df.columns:
        sector_perf = (
            df.groupby("Sector")["Ret_90D"].mean().sort_values(ascending=False)
        )
    ...
    sentiment = pd.to_numeric(
        df.get("sentiment_score", df.get("Sentiment_Raw", np.nan)), errors="coerce"
    )
