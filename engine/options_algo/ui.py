...
from options_algo.logic import (
    fetch_option_chain,
...
def render(broker_choice="Zerodha"):
    ...
    if "NSE" in underlying or ".NS" in underlying:
        st.sidebar.info(
            "⚠️ NSE Options API restricted; using high-fidelity math synthetic data for algo testing."
        )
    ...
    st.dataframe(
        chain_df[["Strike", "Type", "IV", "Delta", "OI", "Premium"]], width="stretch"
    )
    st.download_button(
        "⬇️ Export Options CSV",
        chain_df.to_csv(index=False).encode("utf-8"),
        "options_chain.csv",
        "text/csv",
    )
    ...
    chain_df["Exec"] = chain_df.apply(
        lambda r: (
            generate_zerodha_url(underlying, lots, "BUY")
            if broker_choice == "Zerodha"
            else generate_dhan_url(underlying, lots, transaction_type="BUY")
        ),
        axis=1,
    )
    ...
    if not strat_df.empty:
        ...
        if not recs.empty:
            best = recs.iloc[0]
            st.success(
                f"**Top Pick:** {best['Strategy']} | **Entry:** {best['Entry']} | **Target:** {best['Target Profit']} | **SL:** {best['Stop Loss']}"
            )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name=picked,
                line=dict(color="yellow" if "Long" in picked else "blue"),
            )
        )
        fig.add_hline(y=0, line_width=1, line_dash="dash", line_color="gray")
        fig.update_layout(
            title=f"Strategy Payoff: {picked}",
            xaxis_title="Underlying Price",
            yaxis_title="P&L (Premium)",
        )
