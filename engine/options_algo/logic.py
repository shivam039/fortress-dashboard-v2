...
def calculate_greeks(
    spot: float, strike: float, t: float, rate: float, iv: float, kind: str
):
...
def fetch_option_chain(symbol: str, expiry: str):
    ...
    t = max(
        (datetime.strptime(expiry, "%Y-%m-%d") - datetime.now()).days / 365.0, 1 / 365
    )
    ...
    combined["IV"] = (
        pd.to_numeric(combined["IV"], errors="coerce").fillna(0.2).clip(0.01, 3)
    )
    greeks = combined.apply(
        lambda r: calculate_greeks(
            spot, float(r["Strike"]), t, 0.06, float(r["IV"]), r["Type"]
        ),
        axis=1,
    ).apply(pd.Series)
...
def scan_strategies(chain_df: pd.DataFrame, oi_threshold: int = 10000):
    ...
    atm_strike = chain_df.iloc[
        (chain_df["Strike"] - chain_df["Strike"].median()).abs().argsort()
    ].iloc[0]["Strike"]
    ...
    strangle_pe = (
        pe[pe["Strike"] < atm_strike].sort_values("Strike", ascending=False).head(1)
    )
    ...
    strangle_premium = float(
        strangle_ce["Premium"].sum() + strangle_pe["Premium"].sum()
    )
        ...
        strategies.append(
            {
                ...
                "Max Risk": f"Limited to {straddle_premium:.2f}",
                "Premium": -straddle_premium,
                "IV": atm_iv,
            }
        )
        strategies.append(
            {
                ...
                "Category": "Directional Expansion",
                "Recommendation": "Recommended",
                ...
                "Max Risk": f"Limited to {strangle_premium:.2f}",
                "Premium": -strangle_premium,
                "IV": atm_iv,
            }
        )
...
def payoff_curve(strikes: np.ndarray, strategy: str, premium: float, atm: float):
    ...
    if "Short Strangle" in strategy:
        return (
            premium
            - np.maximum(0, strikes - (atm + width))
            - np.maximum(0, (atm - width) - strikes)
        )
